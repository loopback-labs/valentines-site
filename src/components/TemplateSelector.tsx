...
export default function TemplateSelector({
  ...
  return <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
      {templates.map(template => <Card key={template.id} onClick={() => onSelect(template.id)} className={`cursor-pointer transition-all hover-grow ${selected === template.id ? "border-primary ring-2 ring-primary/20 bg-primary/5" : "border-border hover:border-primary/50"}`}>
          <CardContent className="p-5">
            <div className="flex items-start justify-between mb-3">
              ...
              {selected === template.id && <div className="bg-primary text-primary-foreground rounded-full p-1">
